...
- **Queues**: customer-facing only. Take the queues whose name matches `{Brand} - {Function}` where Function ∈ {Activation, Billing, Complaints, General, Retention, Sales, Technical Support} and Brand ∈ {Acme, OnePass, Members Mobile, Spriggy}. Skip Holding / Internal / Jira / Outbound Email / Documents / Supervisor and any ZZZ_ test queues.
...
- `business_unit_id`: in the Prvidr tenant the Acme+OnePass BU is `a3ad1347-a0e8-44ab-b216-807005b60680`. If the period covers a different brand mix or you're unsure, run `list_management_units` and pick the BU whose MUs serve the queues you're reporting on.
- `management_unit_ids`: in the Prvidr tenant pass `["1c459932-435f-488a-9223-16c00624c36a"]` (Agents_Acme_Catch). Pass an empty list to auto-discover from the user list.
