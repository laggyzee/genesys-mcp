...
| [`cc-daily-brief/`](cc-daily-brief/) | (v0.7) Start-of-day brief for supervisors. Composes yesterday's headline KPIs (SL today vs rolling-7-day median), worst routes, flagged agents (voice AHT excess), unresolved-repeater callback list, and adherence flags into a one-page HTML that fits a laptop screen. |
| [`mcp-reconcile/`](mcp-reconcile/) | (v0.8) Release-time reconciliation checklist. Pulls the MCP tool stack for a known period and outputs a Markdown checklist with side-by-side "MCP says X — verify in Genesys UI here" rows. Run before each release and after material refactors; pairs with `make test` for confidence in correctness. |
| [`genesys-tenant-setup/`](genesys-tenant-setup/) | Per-tenant config wizard. Auto-discovers brand list, queue naming pattern, WFM units, pre-break presence, specialist roles, AHT baselines + timezone from the read-only OAuth client; interviews for the rest; writes a validated `~/.config/genesys-mcp/tenant.yaml`. Run once per tenant — all the other skills depend on this file. |
