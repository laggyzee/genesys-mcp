...
> **v1.24.0 — September 2026:** **Repeat Contacts & FCR.** New `get_repeat_contact_report` tool reports the Repeat Contact Rate and the derived **FCR (Amaysim methodology)** (= 1 − repeat rate) for every window × channel — 7 / 45 days (configurable) × Voice / Messaging / Combined — with cross-channel customer identity (External Contact link → merge/canonical resolution → identifier lookup → normalised raw identifier), a per-channel identity **coverage** block, a lookback that matches but never counts, optional by-day / by-queue breakdowns and a repeat-conversation drill-down. Scope and identity rules live in a runtime-editable `repeatContactReport` config (`get_repeat_contact_config` / `set_repeat_contact_config`, plus `GENESYS_MCP_REPEAT_*` env pins). Settled day-slices and contact resolutions are cached on disk so adjacent periods don't refetch the lookback. 783 tests; 58 tools.
>
> **v1.23.0 — September 2026:** **brand-level sentiment.** New `sentiment_summary` tool answers *"what was each brand's sentiment last week, and did it move?"* in one call — wraps the transcript-aggregates endpoint (`oSentimentScore` + customer phrase counts) grouped by queue × media, rolls up to brand via tenant.yaml `queues.name_pattern` (skip-listed queues excluded), fires the prior period in parallel for `delta_sentiment`, and keeps no-queue bot/flow rows in a separate `unattributed` block instead of dragging brand means down. Soft-fails on 403 naming the exact scope. 708 tests; 55 tools.
>
> **v1.20.0 — July 2026:** **participant-attribute search rewritten against the endpoint's real contract.** The Genesys endpoint only supports conversationId/startTime/endTime/divisionId as searchable fields, so the old attribute-criterion request drew a 400 "Search not supported." on every call once the product flag was enabled. `search_conversations_by_attribute` now pulls startTime windows (4h, or 12h for spans over a week; cursor-paginated, newest first, deduplicated) and filters attributes client-side: "key exists with any value" is the default (no more 0–10 enumeration silently dropping "9.0"/"N/A"/non-0–10 scales), a new `available_keys` block lists every key actually seen (with a "did you mean" note on case mismatches), NPS detection tolerates whole-number decimals and no-response sentinels, and queue/agent row ids come from a batched analytics details lookup (best-effort). 30-day retention clamping, a page budget, and `conversations_scanned`/`truncated` keep the limits honest. Rebuild-only — same tool name/count, output additive. 679 tests; 52 tools.
>
> **v1.19.0 — July 2026:** **recent conversation-detail fallback.** When the async conversations/details archive watermark is behind, `repeat_caller_report` and `repeat_caller_deep_dive` now paginate the synchronous details query and accept it only when every reported `totalHits` row was retrieved exactly once. The tools expose whether the result is complete, provisional, archived, which endpoint supplied it, and the pagination validation. Daily-brief consumers can therefore include yesterday&rsquo;s repeat-caller section while retaining the later authoritative archive repair.
>
> **v1.18.0 — July 2026:** **fresh-presence fallback + utilization correctness.** When Genesys' archived users/details watermark is behind, `presence_sessions` and `break_overrun_report` now use the recent synchronous details endpoint and accept it only after reconciling active-user durations against qualified user aggregates; the response says whether data is complete, provisional, archived, and which source was used. `agent_utilization` no longer groups by the invalid `routingStatus` dimension: it groups by `userId` and reads routing/presence categories from metric qualifiers, restoring agent effectiveness and occupancy comparisons. Conversation-detail watermark fields survive summary mode. Startup logs and `mcp_health_check` now expose the installed MCP version so stale deployed images are obvious. See the full v1.18 notes for the source-selection, reconciliation, and failure contracts.
>
...
| `get_repeat_contact_report` | (v1.24) **Repeat Contacts & FCR.** `period_start` / `period_end` (local dates, inclusive) → one row per window × channel: `total_contacts`, `repeat_contacts`, `repeat_rate`, `fcr` (**FCR (Amaysim methodology)** = 1 − repeat rate, derived not measured), `unidentified_contacts`, `raw_key_contacts`, identity `coverage`, `config_used`, `warnings[]`. A contact is one conversation; it is a repeat if the same customer had *any* prior in-scope contact within N local calendar days. Voice / Messaging match same-channel priors only, Combined matches either. Lookback (period_start − max window) is fetched for matching only. Optional `breakdown: ["day","queue"]` and `drilldown`. Unspecified params fall back to the `repeatContactReport` config. Includes a `methodology` string. See [Repeat Contacts & FCR](#repeat-contacts--fcr-v124). |
| `get_repeat_contact_config` | (v1.24) Effective `repeatContactReport` config + which keys are pinned by `GENESYS_MCP_REPEAT_*` env vars. |
| `set_repeat_contact_config` | (v1.24) Deep-merge a partial update into the stored defaults — no redeploy. Validates the whole config first (windows must be positive integers, unknown keys rejected); writes a local JSON file only, never Genesys. |
...
### Repeat Contacts & FCR (v1.24)

`get_repeat_contact_report` answers *"what share of contacts were repeats, and what FCR does that imply?"* for every combination of window (default 7 and 45 days) and channel (Voice, Messaging, Combined).

**Definitions.** A *contact* is one Genesys conversation, counted once however many sessions, segments or agents it had. A contact is a *repeat* if the same customer had any prior in-scope contact within the preceding N days — any reason, no intent or wrap-up matching — and the repeat is attributed to the later contact. `repeat_rate = repeat_contacts / total_contacts` for that channel and period; **FCR (Amaysim methodology) = 1 − repeat_rate**. It is derived, not measured: no survey or disposition is involved. Voice-only and Messaging-only rates need the prior contact on the same channel; Combined accepts a prior on either (a call followed by a message within N days is a repeat).

**Calculation rules.** Contacts from `period_start − max(window)` to `period_start` are fetched for matching only and never counted. Chains count each link. Two contacts on one day make the second a repeat. A contact never matches itself (conversations are de-duplicated by id). Day boundaries use the configured timezone, and "within N days" compares local calendar dates (a gap of exactly N days is a repeat, N + 1 is not). Scope (direction, abandoned, queues) applies to prior contacts too.

**Customer identity.** Every contact resolves to one `customer_key`, in order: (1) the participant's `externalContactId`, resolved to the canonical surviving contact when merged; (2) an External Contact lookup on the normalised raw identifier — first against identifiers already seen on a linked conversation in the same run (free), then the identifier-lookup API, then a contact search accepted only on exactly one match; (3) the normalised raw identifier itself (E.164 phone with a default country, email, web-messaging user id, social handle) — reported as `raw_key_contacts`, because raw keys rarely link a call to a message; (4) no identifier → `unidentified_contacts`: in the denominator, never a repeat. Every result carries `coverage` (count + % per method, per channel), and Combined rows carry a `combined_rate_likely_understated` warning when the External Contact share is below `identity.low_coverage_warning_pct`.

**Config — section `repeatContactReport`.** Layered: built-in defaults → a `repeatContactReport:` mapping in tenant.yaml → the runtime store written by `set_repeat_contact_config` (`$GENESYS_MCP_REPEAT_CONTACT_CONFIG`, else `repeat-contact-report.json` beside tenant.yaml) → `GENESYS_MCP_REPEAT_*` env pins (which win, and are reported as `env_overridden_keys`).

| Key | Default | Env override |
|---|---|---|
| `direction` | `inbound` (`outbound`, `all`) | `GENESYS_MCP_REPEAT_DIRECTION` |
| `include_abandoned` | `false` — queued but never handled by an agent | `…_INCLUDE_ABANDONED` |
| `include_pre_queue` | `false` — ended in IVR / bot before any queue or agent | `…_INCLUDE_PRE_QUEUE` |
| `media_types.voice` / `.messaging` | `["voice"]` / `["message"]` (all message subtypes) | `…_VOICE_MEDIA_TYPES`, `…_MESSAGING_MEDIA_TYPES` |
| `queue_ids` | `[]` = all queues | `…_QUEUE_IDS` |
| `windows_days` | `[7, 45]` — positive integers, custom windows allowed | `…_WINDOWS_DAYS` |
| `timezone` | `Australia/Sydney` | `…_TIMEZONE` |
| `identity.mode` | `external_contact_first` (or `raw_only`) | `…_IDENTITY_MODE` |
| `identity.enable_lookup` | `true` — identity step 2; adds API volume | `…_IDENTITY_ENABLE_LOOKUP` |
| `identity.resolve_canonical` | `true` — merge resolution via bulk contact fetch | `…_IDENTITY_RESOLVE_CANONICAL` |
| `identity.max_lookups_per_run` | `5000` — period contacts are looked up first | `…_IDENTITY_MAX_LOOKUPS` |
| `identity.phone_normalisation` | `{format: "E.164", default_country: "AU"}` | `…_PHONE_DEFAULT_COUNTRY` |
| `identity.messaging_keys` | `[email, sms, web_messaging_user, social]` | `…_MESSAGING_KEYS` |
| `identity.low_coverage_warning_pct` | `70` | — |
| `drilldown_max_rows` | `2000` per window × channel | — |
| `cache.*` | `enabled: true`, `settled_after_hours: 24`, `retention_days: 120`, `identity_ttl_hours: 24` | `…_CACHE_ENABLED`, `…_CACHE_DIR` |

**Performance and caching.** Conversations are fetched one local day at a time (async details jobs once the archive watermark covers the day, validated synchronous paging before that), fully paged, and reduced to slim records straight away. A cold 45-day lookback walks ~50 day-slices and resolves every distinct External Contact in batches of 50, so the first run can take several minutes on a busy tenant — give the MCP client a generous timeout. Settled, complete days and contact resolutions are cached under `$GENESYS_MCP_CACHE_DIR` (default `~/.cache/genesys-mcp/repeat-contacts/<tenant hash>/`), so adjacent periods and re-runs only fetch new days. The cache contains customer identifiers: files are written `0600` in a `0700` directory, namespaced per OAuth client, and pruned after `cache.retention_days`. Set `cache.enabled: false` to opt out.

**Scopes.** `analytics:conversationDetail:view`; External Contact resolution also needs `externalContacts:contact:view` (`external-contacts:readonly`) and degrades to link + raw-key identity with an `external_contacts_unavailable` warning without it.
...
| `sentiment_summary` | (v1.23) Aggregate sentiment per queue × media **and per brand** over an interval, with prior-period delta. One transcript-aggregates call, not an N+1 walk. Brand rollup from tenant.yaml `queues.name_pattern`; no-queue bot/flow rows surfaced separately as `unattributed`. Needs `analytics:speechAndTextAnalyticsAggregates:view` (bundled into `analytics:readonly` on tenants verified so far). |
